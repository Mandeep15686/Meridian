...
ALLOWED_MIME_TYPES = {
    ...
    "application/msword", "text/plain", "text/html", "text/markdown",
...
async def submit_job(
    ...
    # Validate inputs
    if not files:
        raise HTTPException(status_code=400, detail={"code": "no_files", "message": "At least one file required"})
    if len(files) > settings.MAX_FILES_PER_JOB:
        raise HTTPException(
            status_code=400,
            detail={"code": "too_many_files", "message": f"Max {settings.MAX_FILES_PER_JOB} files per job"},
        )
    ...
    for upload in files:
        ...
        if len(content) > settings.max_file_size_bytes:
            raise HTTPException(
                status_code=413,
                detail={"code": "file_too_large", "message": f"{upload.filename} exceeds {settings.MAX_FILE_SIZE_MB} MB"},
            )
        ...
        if detected_mime not in ALLOWED_MIME_TYPES and declared_mime not in ALLOWED_MIME_TYPES:
            raise HTTPException(
                status_code=415,
                detail={"code": "unsupported_file_type", "message": f"File type not supported: {detected_mime}"},
            )
...
async def get_job_status(
    ...
    progress_pct = min(99, int(completed / stage_count * 100)) if job.status == JobStatus.PROCESSING else None
    ...
    return JobStatusResponse(
        ...
        error={"code": job.error_code, "message": job.error_message, "stage": job.error_stage, "retry_count": job.retry_count}
...
async def get_report(
    ...
    if job.status != JobStatus.COMPLETE:
        raise HTTPException(status_code=409, detail={"code": "job_not_complete", "message": f"Job status: {job.status.value}"})
    ...
    gaps_result = await db.execute(
        select(ComplianceGap).where(ComplianceGap.job_id == job_id).order_by(ComplianceGap.display_order)
    )
    ...
    return ReportResponse(
        ...
        submitted_files=[
            {"filename": f.filename, "modality": f.modality.value, "size_bytes": f.size_bytes}
            for f in files
        ],
...
async def get_corpus_status(
    ...
    for c in corpora:
        ...
        freshness = "stale" if age > stale_threshold else "current"
        corpus_infos.append({
            "id": c.slug,
            "name": c.name,
    ...
    return CorpusStatusResponse(corpora=corpus_infos, total_chunks=total_chunks, index_last_rebuilt=None)
...
async def health_check(db: AsyncSession = Depends(get_db)) -> HealthResponse:
    ...
    overall = "healthy" if all_healthy else ("degraded" if critical_healthy else "unhealthy")
